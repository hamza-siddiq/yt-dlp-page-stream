...
if echo "$output" | grep -qF "Plugin directories: none"; then
  ...
  echo "Use: yt-dlp-ps  OR  yt-dlp-page-stream  OR  python3 -m yt_dlp  OR  scripts/yt-dlp-with-plugin.sh" >&2
...
echo "OK: plugin loaded. For downloads use yt-dlp-ps or yt-dlp-page-stream (not bare yt-dlp unless same Python)."
