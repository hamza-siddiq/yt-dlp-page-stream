...
if echo "$output" | grep -qF "Plugin directories: none"; then
  ...
  echo "Use: page-stream-yt-dlp  OR  python3 -m yt_dlp  OR  scripts/yt-dlp-with-plugin.sh" >&2
...
echo "OK: plugin loaded. For downloads use page-stream-yt-dlp (not bare yt-dlp unless same Python)."
